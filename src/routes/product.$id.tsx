...
function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-b py-3" style={{ borderColor: "#E0E0E0" }}>
      <button
        onClick={() => setO(!o)}
        className="w-full flex justify-between items-center text-[13px] font-bold uppercase"
        style={{ color: "#333" }}
      >
        {title}
        {o ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
      </button>
      {o && (
    ...
    <div className="max-container py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="flex flex-col gap-2">
            {productThumbs.map((t, i) => (
              <button
                key={i}
                onClick={() => setImgIdx(i)}
                className="w-[100px] h-[130px] overflow-hidden"
                style={{ border: imgIdx === i ? "2px solid #E31E24" : "1px solid #E0E0E0" }}
              >
        ...
        <div className="md:pl-12">
          <p className="text-[13px] uppercase" style={{ color: "#888" }}>
            {p.brand}
          </p>
          ...
          <div className="flex items-baseline gap-3 mt-3">
            <span className="text-[22px] font-bold" style={{ color: "#E31E24" }}>
              ???{p.price}
            </span>
            <span className="text-[14px] line-through" style={{ color: "#888" }}>
              ???{p.mrp}
            </span>
            <span className="text-[14px] font-bold" style={{ color: "#E31E24" }}>
              {p.discount}% OFF
            </span>
          </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-[13px] font-bold uppercase" style={{ color: "#333" }}>
              Select Size
            </p>
            <a className="text-[12px]" style={{ color: "#E31E24" }}>
              Size Guide
            </a>
          </div>
          <div className="flex gap-2 mb-5">
            {sizes.map((s) => (
              <button
                key={s}
                onClick={() => setSize(s)}
                className="w-11 h-10 text-[13px] font-medium"
                style={{
                  background: size === s ? "#E31E24" : "#fff",
          ...
          <p className="text-[13px] font-bold uppercase mb-3" style={{ color: "#333" }}>
            Color
          </p>
            ...
            {swatches.map((s) => (
              <button
                ...
                style={{
                  background: s.v,
                  boxShadow:
                    color === s.n ? "0 0 0 2px #fff, 0 0 0 4px #E31E24" : "0 0 0 1px #E0E0E0",
                }}
              />
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 mb-5">
            <span className="text-[13px] font-bold uppercase" style={{ color: "#333" }}>
              Quantity
            </span>
            <div className="flex items-center" style={{ border: "1px solid #E0E0E0" }}>
              <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-9 h-9">
                ???
              </button>
              <span className="w-10 text-center text-[14px]">{qty}</span>
              <button onClick={() => setQty(qty + 1)} className="w-9 h-9">
                +
              </button>
            </div>
          </div>
          ...
          <button
            onClick={() => addToCart(p, { size, color })}
            className="w-full h-12 text-white text-base font-bold uppercase tracking-wider mb-3"
            style={{ background: "#091F13", letterSpacing: "1px" }}
          >
            Add to Bag
          </button>
          ...
          <a
            ...
            className="w-full h-12 flex items-center justify-center gap-3 font-bold text-white uppercase text-sm tracking-wider mb-3 transition hover:opacity-90"
          ...
          <button
            ...
            className="w-full h-12 bg-white text-base font-bold uppercase tracking-wider"
            style={{ color: "#091F13", border: "2px solid #091F13", letterSpacing: "1px" }}
          >
      ...
      <section className="mt-16">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {products.slice(0, 6).map((rp) => (
            <div key={rp.id} className="flex-shrink-0 w-56">
